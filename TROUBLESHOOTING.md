...
### Ошибка: Failed to create user - got an unexpected keyword argument 'username'
...
**Причина:**
Неправильные параметры API для создания пользователей в Remnawave SDK.
...
# 2. Проверьте API параметры
docker exec promo-remna-bot python3 test_user_creation.py
```

**Что было исправлено:**
- Исправлены параметры API: `data_limit` вместо `traffic_limit`
- Убраны неподдерживаемые параметры: `sub_revoked_at`, `sub_last_user_agent`
